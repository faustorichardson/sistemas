...
namespace SisGesCom
{
    public partial class frmTipoBeneficiario : frmBase
    {
        string cModo = "Inicio";

        public frmTipoBeneficiario()
        {
            InitializeComponent();
        }

        private void frmTipoBeneficiario_Load(object sender, EventArgs e)
        {
            Limpiar();
            cModo = "Inicio";
            Botones();
        ...
        private void ProximoCodigo()
        {
            try
            {
                ...
                MyCommand.CommandText = "SELECT count(*) FROM deptobeneficiario";
            ...
            else
            {
                if (cModo == "Nuevo")
                {
                    try
                    {
                        ...
                        myCommand.CommandText = "INSERT INTO deptobeneficiario(deptobeneficiario) values(@deptobeneficiario)";
                        myCommand.Parameters.AddWithValue("@deptobeneficiario", txtDepartamento.Text);
                ...
                else
                {
                    try
                    {
                        ...
                        // Step 3 - Comando a ejecutar
                        myCommand.CommandText = "UPDATE deptobeneficiario SET deptobeneficiario = @deptobeneficiario WHERE id = " + txtCodigo.Text + "";
                        myCommand.Parameters.AddWithValue("@deptobeneficiario", txtDepartamento.Text);                        
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    MyCommand.CommandText = "SELECT deptobeneficiario FROM deptobeneficiario WHERE id = " + txtCodigo.Text + "";
                    ...
                    if (MyReader.HasRows)
                    {
                        while (MyReader.Read())
                        {                            
                            txtDepartamento.Text = MyReader["deptobeneficiario"].ToString();                            
                        }
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                sbQuery.Clear();
                sbQuery.Append("SELECT deptobeneficiario.id, deptobeneficiario.deptobeneficiario");
                sbQuery.Append(" FROM deptobeneficiario ");
                ...
                DataTable dtDeptoBeneficiario = new DataTable();

                // Lleno el data adapter
                myAdapter.Fill(dtDeptoBeneficiario);

                // Cierro el objeto clsConexion
                myclsConexion.Close();

                // Verifico cantidad de datos encontrados
                int nRegistro = dtDeptoBeneficiario.Rows.Count;
                if (nRegistro == 0)
                {
                ...
                else
                {
                    ...
                    cTitulo = "LISTADO DE DEPARTAMENTOS BENEFICIARIOS DE COMBUSTIBLES";

                    //6to Instanciamos nuestro REPORTE
                    //Reportes.ListadoDoctores oListado = new Reportes.ListadoDoctores();
                    //REPORTES.rptClientes orptClientes = new REPORTES.rptClientes();                                        
                    rptListadoDeptoBeneficiario orptListadoDeptoBeneficiario = new rptListadoDeptoBeneficiario();

                    //pasamos el nombre del TITULO del Listado
                    //SumaryInfo es un objeto que se utiliza para leer,crear y actualizar las propiedades del reporte
                    // oListado.SummaryInfo.ReportTitle = cTitulo;
                    orptListadoDeptoBeneficiario.SummaryInfo.ReportTitle = cTitulo;

                    //7mo. instanciamos nuestro el FORMULARIO donde esta nuestro ReportViewer
                    frmPrinter ofrmPrinter = new frmPrinter(dtDeptoBeneficiario, orptListadoDeptoBeneficiario, cTitulo);
