...
{
    public partial class frmRegistroTarjetas : frmBase
    {

        string cModo = "Inicio";
        string militarCedula = "";

        public frmRegistroTarjetas()
        {
            InitializeComponent();
        }
        ...
        private void ProximoCodigo()
        {
            try
            {
                ...
                MyCommand.CommandText = "SELECT count(*) FROM tarjetas";
            ...
            else
            {
                if (cModo == "Nuevo")
                {
                    try
                    {
                        ...
                        myCommand.CommandText = "INSERT INTO tarjetas(fecha, cedula, cantidad) " +
                            "values(@fecha, @cedula, @cantidad)";
                        myCommand.Parameters.AddWithValue("@fecha", dtFecha.Value);
                        myCommand.Parameters.AddWithValue("@cedula", txtCedula.Text);
                        myCommand.Parameters.AddWithValue("@cantidad", txtMonto.Text);
                ...
                else
                {
                    try
                    {
                        ...
                        // Step 3 - Comando a ejecutar
                        myCommand.CommandText = "UPDATE tarjetas SET fecha = @fecha, " +
                            "cedula = @cedula, cantidad = @cantidad WHERE id = " + txtID.Text + "";
                        myCommand.Parameters.AddWithValue("@fecha", dtFecha.Value);
                        myCommand.Parameters.AddWithValue("@cedula", txtCedula.Text);
                        myCommand.Parameters.AddWithValue("@cantidad", txtMonto.Text);
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    // Step 3 - creating the commandtext
                    MyCommand.CommandText = "SELECT id, fecha, cedula, cantidad " +
                        "FROM tarjetas WHERE id = " + txtID.Text + "";
                    ...
                    if (MyReader.HasRows)
                    {
                        while (MyReader.Read())
                        {
                            ...
                            txtMonto.Text = MyReader["cantidad"].ToString();
        ...
        {
            if (txtMonto.Text != "" || txtCedula.Text !="")
            {
                DialogResult Result =
