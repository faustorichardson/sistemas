...
{
    public partial class frmPrintCxP : frmBase
    {
        string cTitulo = "";
        ...
        private void frmPrintCxP_Load(object sender, EventArgs e)
        {
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            ...
            {
                cWhere = cWhere + " AND cxp.status = 1";
                //nombre del TITULO DEL INFORME
                cTitulo = "LISTADO DE CUENTAS POR PAGAR PAGADAS";

            }
            else
            {
                cWhere = cWhere + " AND cxp.status = 0";
                //nombre del TITULO DEL INFORME
                cTitulo = "LISTADO DE CUENTAS POR PAGAR PENDIENTES";
            }
            ...
            try
            {
                ...
                sbQuery.Append("cxp.id, cxp.fecha, cxp.monto, cxp.status, suplidor.nombre as suplidor");
                sbQuery.Append(" FROM cxp ");
                sbQuery.Append(" INNER JOIN suplidor ON cxp.suplidor = suplidor.id ");
                ...
                myAdapter.Fill(dtCxP);
                // Cierro el objeto conexion
                myConexion.Close();

                // Verifico cantidad de datos encontrados
                int nRegistro = dtCxP.Rows.Count;
                if (nRegistro == 0)
                {
                ...
                else
                {
                    ...
                    rptCuentasPorPagar orptCuentasPorPagar = new rptCuentasPorPagar();

                    //pasamos el nombre del TITULO del Listado
                    //SumaryInfo es un objeto que se utiliza para leer,crear y actualizar las propiedades del reporte
                    // oListado.SummaryInfo.ReportTitle = cTitulo;
                    orptCuentasPorPagar.SummaryInfo.ReportTitle = cTitulo;

                    //7mo. instanciamos nuestro el FORMULARIO donde esta nuestro ReportViewer
                    frmPrinter ofrmPrinter = new frmPrinter(dtCxP, orptCuentasPorPagar, cTitulo);
