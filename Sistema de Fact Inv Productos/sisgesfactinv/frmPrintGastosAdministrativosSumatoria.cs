...
{
    public partial class frmPrintGastosAdministrativosSumatoria : frmBase
    {
        public frmPrintGastosAdministrativosSumatoria()
        {
            InitializeComponent();
        }

        private void frmPrintGastosAdministrativosSumatoria_Load(object sender, EventArgs e)
        {
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                sbQuery.Append(" SUM(gastos.monto) as monto, gastos_tipo.desc_tipogasto as categoria");
                sbQuery.Append(" FROM gastos ");
                sbQuery.Append(" INNER JOIN gastos_tipo ON gastos_tipo.idtipogasto = gastos.categoria");
                sbQuery.Append(cWhere);
                sbQuery.Append(" GROUP BY gastos.categoria");
                ...
                DataTable dtGastosAdministrativos = new DataTable();
                // Lleno el data adapter
                myAdapter.Fill(dtGastosAdministrativos);
                // Cierro el objeto conexion
                myConexion.Close();

                // Verifico cantidad de datos encontrados
                int nRegistro = dtGastosAdministrativos.Rows.Count;
                if (nRegistro == 0)
                {
                ...
                else
                {
                    ...
                    cTitulo = "ESTADISTICAS DE GASTOS ADMINISTRATIVOS";

                    //6to Instanciamos nuestro REPORTE
                    //Reportes.ListadoDoctores oListado = new Reportes.ListadoDoctores();
                    rptGastosAdministrativos_Sumatoria orptGastosAdmSumatoria = new rptGastosAdministrativos_Sumatoria();

                    //pasamos el nombre del TITULO del Listado
                    //SumaryInfo es un objeto que se utiliza para leer,crear y actualizar las propiedades del reporte
                    // oListado.SummaryInfo.ReportTitle = cTitulo;
                    orptGastosAdmSumatoria.SummaryInfo.ReportTitle = cTitulo;

                    //7mo. instanciamos nuestro el FORMULARIO donde esta nuestro ReportViewer
                    frmPrinter ofrmPrinter = new frmPrinter(dtGastosAdministrativos, orptGastosAdmSumatoria, cTitulo);
